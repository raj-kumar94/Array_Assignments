...
class CountSteps1
	{
		public static void main(String... s)
		{
			...
			CountSteps1 cs= new CountSteps1();
		...
		while(zero==1)
			{
			zero = cs.checkingForAllZero(b);
			
			if(cs.checkingSum(b)==1)
				{
				count=	cs.divisionByTwo(b,count);  //gadbad!!!! call it only when all elements are even
				} 
			else 
			{
			count=	cs.checkingForOdd(b,count);   
			}
